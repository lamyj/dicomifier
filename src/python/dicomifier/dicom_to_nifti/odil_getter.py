...
def _diffusion_getter(data_set, tag):
	...
	directionality = _getter(value[0],odil.registry.DiffusionDirectionality)[0]
        sensitization = None
        if directionality == "DIRECTIONAL":
	    item = _getter(value[0],odil.registry.DiffusionGradientDirectionSequence)[0]
	    sensitization = _getter(item,odil.registry.DiffusionGradientOrientation)
