...
::set_affected_sop_class(DcmDataset const * dataset)
{
    ...
    std::vector<std::string> const known_uids = {
    ...
    else
    {
        throw DicomifierException(
            "Could not guess affected SOP class from dataset");
    }
}
    ...
    long bytes_count) const
{
    T_ASC_PresentationContextID const presentation_id = 
        ASC_findAcceptedPresentationContextID(this->_association, 
            this->_affected_sop_class.c_str());
    if(presentation_id == 0) 
    {
        throw DicomifierException("No Presentation Context for Find Operation");
    }
    ...
    OFCondition const condition = DIMSE_storeUser(this->_association, 
        presentation_id, &request, NULL, const_cast<DcmDataset*>(dataset),
        callback, data,
        DIMSE_BLOCKING, this->_timeout, &response, NULL,
        NULL, bytes_count);
