...
void DicomDirGenerator::cleanup()
{
...
OFBool DicomDirGenerator::isDicomDirValid() const
{
...
void DicomDirGenerator::createDicomDirBackup(const char *filename)
{
...
void DicomDirGenerator::deleteDicomDirBackup()
{
...
OFCondition DicomDirGenerator::createNewDicomDir(const E_ApplicationProfile profile,
                                                 const char *filename,
...
OFCondition DicomDirGenerator::appendToDicomDir(const E_ApplicationProfile profile,
                                                const char *filename)
{
    OFCondition result = EC_IllegalParameter;
    if (filename != NULL)
...
OFCondition DicomDirGenerator::updateDicomDir(const E_ApplicationProfile profile,
                                              const char *filename)
{
    OFCondition result = EC_IllegalParameter;
    if (filename != NULL)
...
OFCondition DicomDirGenerator::writeDicomDir(const E_EncodingType encodingType,
...
OFBool DicomDirGenerator::isFilenameValid(const char *filename,
...
OFBool DicomDirGenerator::isCharsetValid(const char *charset)
{
    OFBool result = OFTrue;
...
OFCondition DicomDirGenerator::checkSOPClassAndXfer(DcmMetaInfo *metainfo,
                                                    DcmItem *dataset,
                                                    const char *filename)
{
    OFCondition result = EC_IllegalParameter;
...
OFCondition DicomDirGenerator::checkBasicCardiacAttributes(DcmItem *dataset,
                                                           const char *filename)
{
...
OFCondition DicomDirGenerator::checkXrayAngiographicAttributes(DcmItem *dataset,
...
OFCondition DicomDirGenerator::checkDentalRadiographAttributes(DcmItem *dataset,
                                                               const char *filename)
{
    OFCondition result = EC_Normal;
...
OFCondition DicomDirGenerator::checkCTandMRAttributes(DcmItem *dataset,
                                                      const OFString &sopClass,
                                                      const char *filename)
{
    OFCondition result = EC_Normal;
...
OFCondition DicomDirGenerator::checkUltrasoundAttributes(DcmItem *dataset,
...
OFCondition DicomDirGenerator::checkMandatoryAttributes(DcmMetaInfo *metainfo,
                                                        DcmItem *dataset,
                                                        const char *filename)
{
    OFCondition result = EC_IllegalParameter;
...
OFCondition DicomDirGenerator::selectApplicationProfile(const E_ApplicationProfile profile)
{
    OFCondition result = EC_Normal;
...
OFCondition DicomDirGenerator::checkDicomFile(const char *filename,
...
OFCondition DicomDirGenerator::loadAndCheckDicomFile(const char *filename,
                                                     const char *directory,
                                                     DcmFileFormat &fileformat)
{
    OFCondition result = EC_IllegalParameter;
...
OFBool DicomDirGenerator::recordMatchesDataset(DcmDirectoryRecord *record,
                                               DcmItem *dataset)
{
    OFBool result = OFFalse;
    if ((record != NULL) && (dataset != NULL))
...
DcmDirectoryRecord *DicomDirGenerator::findExistingRecord(DcmDirectoryRecord *parent,
...
DcmDirectoryRecord *DicomDirGenerator::buildPatientRecord(DcmDirectoryRecord *record,
                                                          DcmItem *dataset,
                                                          const OFString &sourceFilename)
{
    ...
    if (record != NULL)
    {
        ...
        if (record->error().good())
        {
            ...
            // Add extra patient attributes
            for(std::vector<DcmTagKey>::const_iterator it=this->_patient_extra_attributes.begin();
                it != this->_patient_extra_attributes.end(); ++it)
            {
                copyElementType3(dataset, *it, record, sourceFilename);
            }
        } else {
...
DcmDirectoryRecord *DicomDirGenerator::buildStudyRecord(DcmDirectoryRecord *record,
                                                        DcmItem *dataset,
                                                        const OFString &sourceFilename)
{
    ...
    if (record != NULL)
    {
        ...
        if (record->error().good())
        {
            ...
            // Add extra study attributes
            for(std::vector<DcmTagKey>::const_iterator it=this->_study_extra_attributes.begin();
                it != this->_study_extra_attributes.end(); ++it)
            {
                copyElementType3(dataset, *it, record, sourceFilename);
            }
        } else {
...
DcmDirectoryRecord *DicomDirGenerator::buildSeriesRecord(DcmDirectoryRecord *record,
                                                         DcmItem *dataset,
                                                         const OFString &sourceFilename)
{
    ...
    if (record != NULL)
    {
        ...
        if (record->error().good())
        {
            ...
            // Add extra series attributes
            for(std::vector<DcmTagKey>::const_iterator it=this->_series_extra_attributes.begin();
                it != this->_series_extra_attributes.end(); ++it)
            {
                copyElementType3(dataset, *it, record, sourceFilename);
            }
        } else {
...
DcmDirectoryRecord *DicomDirGenerator::buildOverlayRecord(DcmDirectoryRecord *record,
                                                          DcmItem *dataset,
...
DcmDirectoryRecord *DicomDirGenerator::buildModalityLutRecord(DcmDirectoryRecord *record,
                                                              DcmItem *dataset,
                                                              const OFString &referencedFileID,
                                                              const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildVoiLutRecord(DcmDirectoryRecord *record,
                                                         DcmItem *dataset,
                                                         const OFString &referencedFileID,
                                                         const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildCurveRecord(DcmDirectoryRecord *record,
                                                        DcmItem *dataset,
                                                        const OFString &referencedFileID,
                                                        const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildStructReportRecord(DcmDirectoryRecord *record,
                                                               DcmItem *dataset,
                                                               const OFString &referencedFileID,
                                                               const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildPresentationRecord(DcmDirectoryRecord *record,
                                                               DcmItem *dataset,
                                                               const OFString &referencedFileID,
                                                               const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildWaveformRecord(DcmDirectoryRecord *record,
                                                           DcmItem *dataset,
                                                           const OFString &referencedFileID,
                                                           const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildRTDoseRecord(DcmDirectoryRecord *record,
                                                         DcmItem *dataset,
                                                         const OFString &referencedFileID,
                                                         const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildRTStructureSetRecord(DcmDirectoryRecord *record,
                                                                 DcmItem *dataset,
                                                                 const OFString &referencedFileID,
                                                                 const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildRTPlanRecord(DcmDirectoryRecord *record,
                                                         DcmItem *dataset,
                                                         const OFString &referencedFileID,
                                                         const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildRTTreatmentRecord(DcmDirectoryRecord *record,
                                                              DcmItem *dataset,
                                                              const OFString &referencedFileID,
                                                              const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildStoredPrintRecord(DcmDirectoryRecord *record,
                                                              DcmItem *dataset,
                                                              const OFString &referencedFileID,
                                                              const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildKeyObjectDocRecord(DcmDirectoryRecord *record,
                                                               DcmItem *dataset,
                                                               const OFString &referencedFileID,
                                                               const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildRegistrationRecord(DcmDirectoryRecord *record,
                                                               DcmItem *dataset,
                                                               const OFString &referencedFileID,
                                                               const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildFiducialRecord(DcmDirectoryRecord *record,
                                                           DcmItem *dataset,
                                                           const OFString &referencedFileID,
                                                           const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildRawDataRecord(DcmDirectoryRecord *record,
                                                          DcmItem *dataset,
                                                          const OFString &referencedFileID,
                                                          const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildSpectroscopyRecord(DcmDirectoryRecord *record,
                                                               DcmItem *dataset,
                                                               const OFString &referencedFileID,
                                                               const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildEncapDocRecord(DcmDirectoryRecord *record,
                                                           DcmItem *dataset,
                                                           const OFString &referencedFileID,
                                                           const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildValueMapRecord(DcmDirectoryRecord *record,
                                                           DcmItem *dataset,
                                                           const OFString &referencedFileID,
                                                           const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildHangingProtocolRecord(DcmDirectoryRecord *record,
                                                                  DcmItem *dataset,
                                                                  const OFString &referencedFileID,
                                                                  const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildStereometricRecord(DcmDirectoryRecord *record,
                                                               DcmItem * /* dataset */ ,
                                                               const OFString &referencedFileID,
                                                               const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildPaletteRecord(DcmDirectoryRecord *record,
                                                          DcmItem *dataset,
                                                          const OFString &referencedFileID,
                                                          const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildSurfaceRecord(DcmDirectoryRecord *record,
                                                          DcmItem *dataset,
                                                          const OFString &referencedFileID,
                                                          const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildMeasurementRecord(DcmDirectoryRecord *record,
                                                              DcmItem *dataset,
                                                              const OFString &referencedFileID,
                                                              const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildImplantRecord(DcmDirectoryRecord *record,
                                                          DcmItem *dataset,
                                                          const OFString &referencedFileID,
                                                          const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildImplantGroupRecord(DcmDirectoryRecord *record,
                                                               DcmItem *dataset,
                                                               const OFString &referencedFileID,
                                                               const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildImplantAssyRecord(DcmDirectoryRecord *record,
                                                              DcmItem *dataset,
                                                              const OFString &referencedFileID,
                                                              const OFString &sourceFilename)
{
...
DcmDirectoryRecord *DicomDirGenerator::buildImageRecord(DcmDirectoryRecord *record,
                                                        DcmItem *dataset,
                                                        const OFString &referencedFileID,
                                                        const OFString &sourceFilename)
{
...
OFBool DicomDirGenerator::getIconFromFile(const OFString &filename,
                                          Uint8 *pixel,
...
OFBool DicomDirGenerator::getIconFromDataset(DcmItem *dataset,
                                             Uint8 *pixel,
                                             const unsigned long count,
                                             const unsigned int width,
                                             const unsigned int height)
...
OFCondition DicomDirGenerator::addIconImage(DcmDirectoryRecord *record,
                                            DcmItem *dataset,
...
DcmDirectoryRecord *DicomDirGenerator::addRecord(DcmDirectoryRecord *parent,
                                                 const E_DirRecType recordType,
                                                 DcmItem *dataset,
                                                 const OFString &referencedFileID,
                                                 const OFString &sourceFilename)
...
OFBool DicomDirGenerator::checkReferencedSOPInstance(DcmDirectoryRecord *record,
                                                     DcmItem *dataset,
                                                     const OFString &referencedFileID,
                                                     const OFString &sourceFilename)
{
...
void DicomDirGenerator::inventMissingAttributes(DcmDirectoryRecord *parent,
...
void DicomDirGenerator::inventMissingStudyLevelAttributes(DcmDirectoryRecord *parent)
{
    if (parent != NULL)
    {
        DcmDirectoryRecord *record = NULL;
...
void DicomDirGenerator::inventMissingSeriesLevelAttributes(DcmDirectoryRecord *parent)
{
    if (parent != NULL)
    {
        DcmDirectoryRecord *record = NULL;
...
void DicomDirGenerator::inventMissingInstanceLevelAttributes(DcmDirectoryRecord *parent)
{
    if (parent != NULL)
    {
        DcmDirectoryRecord *record = NULL;
...
OFCondition DicomDirGenerator::addDicomFile(const char *filename,
                                            const char *directory)
{
    OFCondition result = EC_IllegalParameter;
...
OFCondition DicomDirGenerator::setFilesetDescriptor(const char *filename,
                                                    const char *charset)
{
    OFCondition result = EC_IllegalCall;
    if (DicomDir != NULL)
...
OFCondition DicomDirGenerator::setIconSize(const unsigned int size)
{
    OFCondition result = EC_IllegalParameter;
    /* check valid range */
    if ((size > 0) && (size <= 256))
...
OFCondition DicomDirGenerator::setIconPrefix(const char *prefix)
{
    IconPrefix = prefix;
    return EC_Normal;
}
...
OFCondition DicomDirGenerator::setDefaultIcon(const char *filename)
{
...
OFBool DicomDirGenerator::enableAbortMode(const OFBool newMode)
{
    /* save current mode */
...
OFBool DicomDirGenerator::enableMapFilenamesMode(const OFBool newMode)
{
    /* save current mode */
...
OFBool DicomDirGenerator::enableInventMode(const OFBool newMode)
{
    /* save current mode */
...
OFBool DicomDirGenerator::enableInventPatientIDMode(const OFBool newMode)
{
    /* save current mode */
...
OFBool DicomDirGenerator::enableRetiredSOPClassSupport(const OFBool newMode)
{
    /* save current mode */
...
OFBool DicomDirGenerator::enableIconImageMode(const OFBool newMode)
{
    /* save current mode */
...
OFBool DicomDirGenerator::disableBackupMode(const OFBool newMode)
{
    /* save current mode */
...
OFBool DicomDirGenerator::disableEncodingCheck(const OFBool newMode)
{
    /* save current mode */
...
// for particular application profiles
OFBool DicomDirGenerator::disableResolutionCheck(const OFBool newMode)
{
    /* save current mode */
...
// for particular application profiles
OFBool DicomDirGenerator::disableTransferSyntaxCheck(const OFBool newMode)
{
    /* save current mode */
...
OFBool DicomDirGenerator::disableConsistencyCheck(const OFBool newMode)
{
    /* save current mode */
...
OFBool DicomDirGenerator::addImageSupport(DicomDirImagePlugin *plugin)
{
...
void DicomDirGenerator::printUnexpectedValueMessage(const DcmTagKey &key,
                                                    const char *filename,
...
void DicomDirGenerator::printRequiredAttributeMessage(const DcmTagKey &key,
                                                      const char *filename,
...
void DicomDirGenerator::printAttributeErrorMessage(const DcmTagKey &key,
...
void DicomDirGenerator::printRecordErrorMessage(const OFCondition &error,
                                                const E_DirRecType recordType,
                                                const char *operation)
{
    if (error.bad())
...
const char *DicomDirGenerator::getProfileName(const E_ApplicationProfile profile)
{
    const char *result = "";
    switch(profile)
    {
...
std::vector<DcmTagKey> const &
DicomDirGenerator
::getPatientExtraAttributes() const
{
    return this->_patient_extra_attributes;
}

void
DicomDirGenerator
::setPatientExtraAttributes(std::vector<DcmTagKey> const & attributes)
{
    this->_patient_extra_attributes = attributes;
}

std::vector<DcmTagKey> const &
DicomDirGenerator
::getStudyExtraAttributes() const
{
    return this->_study_extra_attributes;
}

void
DicomDirGenerator
::setStudyExtraAttributes(std::vector<DcmTagKey> const & attributes)
{
    this->_study_extra_attributes = attributes;
}

std::vector<DcmTagKey> const &
DicomDirGenerator
::getSeriesExtraAttributes() const
{
    return this->_series_extra_attributes;
}

void
DicomDirGenerator
::setSeriesExtraAttributes(std::vector<DcmTagKey> const & attributes)
{
    this->_series_extra_attributes = attributes;
}

// copy contents of specified file
OFBool DicomDirGenerator::copyFile(const char *fromFilename,
...
OFBool DicomDirGenerator::warnAboutInconsistentAttributes(DcmDirectoryRecord *record,
                                                          DcmItem *dataset,
                                                          const OFString &sourceFilename,
...
OFBool DicomDirGenerator::checkFilesetID(const OFString &filesetID)
{
    OFBool result = OFTrue;
    if (!filesetID.empty())
    {
...
OFBool DicomDirGenerator::checkExists(DcmItem *dataset,
                                      const DcmTagKey &key,
                                      const char *filename)
{
...
OFBool DicomDirGenerator::checkExistsWithValue(DcmItem *dataset,
                                               const DcmTagKey &key,
                                               const char *filename)
{
...
OFBool DicomDirGenerator::checkExistsWithStringValue(DcmItem *dataset,
                                                     const DcmTagKey &key,
...
OFBool DicomDirGenerator::checkExistsWithIntegerValue(DcmItem *dataset,
                                                      const DcmTagKey &key,
                                                      const long value,
                                                      const char *filename,
                                                      const OFBool reject)
...
OFBool DicomDirGenerator::checkExistsWithMinMaxValue(DcmItem *dataset,
                                                     const DcmTagKey &key,
                                                     const long min,
                                                     const long max,
                                                     const char *filename,
...
OFString &DicomDirGenerator::getStringFromDataset(DcmItem *dataset,
                                                  const DcmTagKey &key,
                                                  OFString &result,
...
OFString &DicomDirGenerator::getStringComponentFromDataset(DcmItem *dataset,
                                                           const DcmTagKey &key,
                                                           OFString &result,
...
OFString &DicomDirGenerator::getStringFromFile(const char *filename,
                                               const DcmTagKey &key,
                                               OFString &result,
                                               OFBool searchIntoSub)
{
...
void DicomDirGenerator::copyElement(DcmItem *dataset,
                                    const DcmTagKey &key,
...
void DicomDirGenerator::copyStringWithDefault(DcmItem *dataset,
                                              const DcmTagKey &key,
                                              DcmDirectoryRecord *record,
                                              const OFString &sourceFilename,
...
OFBool DicomDirGenerator::compareStringAttributes(DcmItem *dataset,
...
OFBool DicomDirGenerator::compareSequenceAttributes(DcmItem *dataset,
                                                    DcmTagKey &key,
                                                    DcmDirectoryRecord *record,
                                                    const OFString &sourceFilename)
{
...
void DicomDirGenerator::setDefaultValue(DcmDirectoryRecord *record,
                                        const DcmTagKey &key,
