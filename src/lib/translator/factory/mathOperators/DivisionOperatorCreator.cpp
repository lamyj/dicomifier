...
#include "DivisionOperatorCreator.h"
...
static unsigned int const registration = TranslatorFactory::get_instance().register_<DivisionOperatorCreator>(); 
...
{    
    ...
    TranslatorDivisionOperatorCreator action;
    action.pttag = pttag;
    
    dicomifier::vr_dispatch(action, evr);
    
    return action.divisionoperator;
}

template<>
void 
DivisionOperatorCreator::TranslatorDivisionOperatorCreator
::run<EVR_SQ>() const
{
...
::run() const
{
    divisionoperator = dicomifier::translator::DivisionOperator<VR>::New(pttag);
}
