...
var indexGenerator = new dicomifier.frameIndexGenerator(brukerDataset);

while (indexGenerator.done() == false) {
    var dicomDataset = {};

    // Patient module
    toDicom(dicomDataset, 'PatientName', 
    ...
    toDicom(dicomDataset, 'StudyInstanceUID', 
    ...
    toDicom(dicomDataset, 'StudyDescription', 
    ...
    toDicom(dicomDataset, 'SeriesTime', brukerDataset.VisuSeriesDate, 2,
            dateTimeMapper('time'));
    toDicom(dicomDataset, 'PerformingPhysicianName', 
    ...
    toDicom(dicomDataset, 'FrameOfReferenceUID', 
    ...
    toDicom(dicomDataset, 'InstanceNumber', 
            [ indexGenerator.currentStep + 1 ], 2);
    toDicom(dicomDataset, 'ImageType', [ 'ORIGINAL', 'PRIMARY' ], 1);
    toDicom(dicomDataset, 'AcquisitionDate', brukerDataset.VisuAcqDate, 
            3, dateTimeMapper('date'));
    toDicom(dicomDataset, 'AcquisitionTime', brukerDataset.VisuAcqDate, 
            3, dateTimeMapper('time'));
    ...
    toDicom(dicomDataset, 'PhotometricInterpretation', 
            [ 'MONOCHROME2' ], 1);
    toDicom(dicomDataset, 'Rows', [ brukerDataset.VisuCoreSize[0] ], 1);
    toDicom(dicomDataset, 'Columns', 
            [ brukerDataset.VisuCoreSize[1] ], 1);
    ...
    toDicom(dicomDataset, 'MRAcquisitionType', brukerDataset.VisuCoreDim, 
            2, function(x) { return x + 'D'; } );
    ...
    toDicom(dicomDataset, 'SpacingBetweenSlices', 
            [ spacingbetweenslice() ], 3);
    ...
    // SOP Common Module
    toDicom(dicomDataset, 'SOPClassUID', 
            [ '1.2.840.10008.5.1.4.1.1.4' ], 1);
    toDicom(dicomDataset, 'SOPInstanceUID',
            [ dcmGenerateUniqueIdentifier() ], 1);
    toDicom(dicomDataset, 'InstanceCreationDate', [ String(new Date()) ], 
            3, dateTimeMapper('date'));
    toDicom(dicomDataset, 'InstanceCreationTime', [ String(new Date()) ], 
            3, dateTimeMapper('time'));
