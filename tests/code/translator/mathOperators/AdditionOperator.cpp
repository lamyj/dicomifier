...
BOOST_AUTO_TEST_CASE(TEST_OK_01)
{
    ...
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldae->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = AS
    auto testfieldas = dicomifier::translator::AdditionOperator<EVR_AS>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldas->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    ...
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldcs->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = DA
    auto testfieldda = dicomifier::translator::AdditionOperator<EVR_DA>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldda->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = DS
    auto testfieldds = dicomifier::translator::AdditionOperator<EVR_DS>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldds->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = DT
    auto testfielddt = dicomifier::translator::AdditionOperator<EVR_DT>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfielddt->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = FL
    auto testfieldfl = dicomifier::translator::AdditionOperator<EVR_FL>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldfl->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = FD
    auto testfieldfd = dicomifier::translator::AdditionOperator<EVR_FD>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldfd->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = IS
    auto testfieldis = dicomifier::translator::AdditionOperator<EVR_IS>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldis->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = LO
    auto testfieldlo = dicomifier::translator::AdditionOperator<EVR_LO>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldlo->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = LT
    auto testfieldlt = dicomifier::translator::AdditionOperator<EVR_LT>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldlt->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    ...
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldpn->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = SH
    auto testfieldsh = dicomifier::translator::AdditionOperator<EVR_SH>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldsh->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = SL
    auto testfieldsl = dicomifier::translator::AdditionOperator<EVR_SL>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldsl->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    ...
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldss->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = ST
    auto testfieldst = dicomifier::translator::AdditionOperator<EVR_ST>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldst->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = TM
    auto testfieldtm = dicomifier::translator::AdditionOperator<EVR_TM>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldtm->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = UI
    auto testfieldui = dicomifier::translator::AdditionOperator<EVR_UI>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldui->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = UL
    auto testfieldul = dicomifier::translator::AdditionOperator<EVR_UL>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldul->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    ...
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldus->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
    
    // Test VR = UT
    auto testfieldut = dicomifier::translator::AdditionOperator<EVR_UT>::New();
    // Pointer exists and class type is AdditionOperator
    BOOST_CHECK_EQUAL(testfieldut->get_class_type(), dicomifier::translator::ECT_AdditionOperator);
}
